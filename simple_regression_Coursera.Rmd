...
<li> The relationship is positive, linear, and moderately strong. One of the potential outliers is a team with approximately 5520 at bats. - This one</li>
...
<ol>
<li> For each additional home run, the model predicts 1.83 more runs, on average. - This one</li>
...
<li> The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met.- This one </li>
</ol>
...
```{r pick-model}
mlb11 %>%
  summarise(cor(runs, strikeouts)^2)

mlb11 %>%
  summarise(cor(runs, at_bats)^2)

```

Just 16% of the variability is explained by the model for strikeouts, compared to 37% for at bats.
...
<li> hits </li>
<li> wins </li>
<li> batting average - This one </li>
</ol>
```{r runs-vs-others}
# at bats
...
hitsm1 <- lm(runs ~ hits, data = mlb11)

ggplot(data = hitsm1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")

ggplot(data = hitsm1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = hitsm1, aes(sample = .resid)) +
  stat_qq()


mlb11 %>%
  summarise(cor(runs, hits)^2)
...
winssm1 <- lm(runs ~ wins, data = mlb11)

ggplot(data = winssm1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")
...
ggplot(data = winssm1, aes(sample = .resid)) +
  stat_qq()


mlb11 %>%
  summarise(cor(runs, wins)^2)
...
batavgm1 <- lm(runs ~ bat_avg, data = mlb11)

ggplot(data = batavgm1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")
...
ggplot(data = batavgm1, aes(sample = .resid)) +
  stat_qq()


mlb11 %>%
  summarise(cor(runs, bat_avg)^2)
...
<ol>
<li> on-base plus slugging (`new_obs`) - This one </li>
...
obsm1 <- lm(runs ~ new_obs, data = mlb11)

ggplot(data = obsm1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")

ggplot(data = obsm1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = obsm1, aes(sample = .resid)) +
  stat_qq()


mlb11 %>%
  summarise(cor(runs, new_obs)^2)
...
slugm1 <- lm(runs ~ new_slug, data = mlb11)

ggplot(data = slugm1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")

ggplot(data = slugm1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = slugm1, aes(sample = .resid)) +
  stat_qq()


mlb11 %>%
  summarise(cor(runs, new_slug)^2)
...
basem1 <- lm(runs ~ new_onbase, data = mlb11)

ggplot(data = basem1, aes(x = .resid)) +
  geom_histogram(binwidth = 25) +
  xlab("Residuals")

ggplot(data = basem1, aes(x = .fitted, y = .resid)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  xlab("Fitted values") +
  ylab("Residuals")

ggplot(data = basem1, aes(sample = .resid)) +
  stat_qq()


mlb11 %>%
  summarise(cor(runs, new_onbase)^2)
